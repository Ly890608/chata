...
if "%BUILD_RELEASE%" == "1" (
    :: /MT
    call :BuildVariant mt "-DCHATA_USE_STATIC_CRT=ON" Release || exit /b

    if "%BUILD_RELEASE_VARIANTS%" == "1" (
        :: /MD
        call :BuildVariant md "-DCHATA_USE_STATIC_CRT=OFF" Release || exit /b
    )
)

if "%BUILD_DEBUG%" == "1" (
    :: MTd
    call :BuildVariant mtd "-DCHATA_USE_STATIC_CRT=ON" Debug || exit /b

    if "%BUILD_RELEASE_VARIANTS%" == "1" (
        :: MDd
        call :BuildVariant mdd "-DCHATA_USE_STATIC_CRT=OFF" Debug || exit /b
    )
)
...
if "%BUILD_RELEASE_VARIANTS%" == "1" (
    cd out\install
    7z a -ttar -so ..\..\chata-release.tar * | 7z a -si ..\chata-windows.tgz
)
